...
public class TranslationService {
    ...
    @Transactional
    public void saveTranslation(long chatId, @NotEmpty String sourcePhrase, @NotEmpty List<String> targetPhrases,
            int messageId) {

        Optional<Translation> translationOpt = translationRepository.findByChatIdAndSourcePhrase(chatId, sourcePhrase);
        if (translationOpt.isPresent()) {
            updateTranslation(translationOpt.get(), targetPhrases, messageId);
        } else {
            createTranslation(chatId, sourcePhrase, targetPhrases, messageId);
        }
    }

    private Translation createTranslation(long chatId, String sourcePhrase, List<String> targetPhrases,
            int messageId) {

        Translation translation = new Translation(chatId, sourcePhrase, targetPhrases);
        translation = translationRepository.save(translation);

        Translation translationToUse = translation;

        executeAfterCommit(
                () -> engineEventsPublisher.publish(
                        new TranslationSavedEventDto(translationToUse.getChatId(), messageId,
                                translationToUse.getSourcePhrase(), translationToUse.getTargetPhrases(), false)));

        return translationToUse;
    }

    private Translation updateTranslation(Translation translation, List<String> targetPhrases, int messageId) {

        translation.setTargetPhrases(targetPhrases);
        translation = translationRepository.save(translation);
        ...
        executeAfterCommit(
                () -> engineEventsPublisher.publish(
                        new TranslationSavedEventDto(translationToUse.getChatId(), messageId,
                                translationToUse.getSourcePhrase(), translationToUse.getTargetPhrases(), true)));
